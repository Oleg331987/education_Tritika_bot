...
💰 <b>Стоимость:</b> 
   <s>7500 руб.</s> 
   <b>4999 руб.</b> (акция до конца января 2026 года)
...
async def handle_get_access(message: Message):
    ...
    # Формируем красивое сообщение с информацией об оплате
    access_info = f"""
<b>🔓 ПРЕДЛОЖЕНИЕ С ОГРАНИЧЕННЫМ СРОКОМ ДЕЙСТВИЯ!</b>

✨ <b>ОСОБАЯ ЦЕНА НА СТАРТЕ:</b>

<s>Обычная стоимость: 7 500 руб.</s>

🎁 <b>АКЦИОННАЯ ЦЕНА: <u>4 999 руб.</u></b>
...
💰 <b>Тип доступа:</b> Полный курс (4 999 руб.)
...
<b>💰 Оплата и доступ:</b>
• <b>🔓 Получить доступ</b> - оплата курса (4 999 руб. до конца января 2026)
...
<b>💰 Цена курса:</b>
• Обычная: 7 500 руб.
• Акционная (до 01.2026): 4 999 руб.
    ...
    if message.content_type == ContentType.TEXT:
        if access_control.is_paid_user(user_id):
            ...
            await message.answer(
                ...
                "1. Оплатите подписку (4 999 руб. до конца января 2026)\n"
                "2. Согласитесь с обработкой персональных данных\n"
                "3. Отправьте чек об оплате в этот чат\n\n"
                f"<b>Ваш ID:</b> <code>{user_id}</code>\n\n"
                "📞 <b>Контакты:</b>\n"
                f"Телефон: {ADDITIONAL_MATERIALS['contacts']['mobile']}\n"
                f"Email: {ADDITIONAL_MATERIALS['contacts']['email']}\n\n"
                "Нажмите '🔓 Получить доступ' для оплаты и получения подробной информации.",
                parse_mode=ParseMode.HTML,
                reply_markup=get_main_keyboard(user_id)
            )
...
async def run_bot_with_retries():
    ...
    while not shutdown_flag and restart_count < max_restarts:
        try:
            ...
            logger.info(f"✅ Стоимость курса: 4 999 руб. (акция до 01.2026)")
...
if __name__ == "__main__":
    try:
        ...
        print(f"💰 Стоимость курса: 4 999 руб. (акция до 01.2026)")
        ...
        print("• QR-код для оплаты 4 999 руб. (акция до 01.2026)")
