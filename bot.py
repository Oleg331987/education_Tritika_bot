...
<b>💡 Ваш следующий шаг — ДЕЙСТВИЕ.</b> Теперь у вас есть знания, поддержка и выгодные условия для первых побед. Мы будем рады помочь вам в этом!
...
• 📚 {len(MODULES)} модулей с теорией и практикой
• 🎧 <b>Аудио-сопровождение к каждому уроку</b>
• 📝 Практические задания
• 📊 Отслеживание прогресса
• <b>📝 Финальный тест</b> для проверки знаний
• 🎁 <b>Специальные подарки для выпускников</b> (8-й день)
...
async def run_bot_with_retries():
    ...
    if not files_ok:
        logger.warning("Некоторые файлы отсутствуют. Бот продолжит работу, но некоторые функции могут быть недоступны.")
    ...
    while not shutdown_flag and restart_count < max_restarts:
        try:
            ...
            logger.info(f"✅ Всего модулей в курсе: {len(MODULES)}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"📊 Всего модулей в курсе: {len(MODULES)}")
        ...
        print(f"• Добавлен 8-й модуль: 🎁 Подарки для выпускников")
        ...
        print("• Автосохранение прогресса каждые 5 минут")
