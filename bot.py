...
# Загружаем прогресс пользователей
...
1. Обратитесь в компанию ООО "БИЗНЕС-ПОДДЕРЖКА",
   сайт - банк-гарантия.рф, телефон: 8 800 600 04 60, e-mail: garant@bzgarant.com
   ***Приглашаем к сотрудничеству! Вы с нами оформляете БГ и получаете вознаграждение по каждой сделке. Пишите «% за БГ» в ТГ @malkovabzgarant и мы расскажем как.***
...
ADDITIONAL_MATERIALS = {
    "links": {
        ...
        "Политика конфиденциальности": "https://tritika.ru/privacy"
    },
    "contacts": {
...
# user_progress уже загружен в начале файла через load_user_progress()
...
async def send_final_summary(message: Message):
    ...
    final_audio_sent = await audio_manager.send_module_audio(message.chat.id, 6, user_id)  # Теперь модуль 7 имеет индекс 6
...
<b>🎯 Теперь ваша очередь действовать! Первый шаг — самый важный!</b>"""
    ...
    # ДОБАВЛЯЕМ ИНФОРМАЦИЮ О ПОДАРКАХ ДЛЯ ВЫПУСКНИКОВ
    gifts_text = """<b>🎁 Подарки для наших выпускников</b>
...
Чтобы поддержать вас на старте, мы предоставляем эксклюзивную скидку 10% на первую услугу <b>из следующего списка</b>:

<b>Сопровождение одного конкретного тендера</b>
• Ваша выгода: Скидка на первое сопровождение
• Идеально для старта. Вы выбираете тендер, а наши эксперты помогут подготовить и подать заявку «под ключ», избегая ошибок.

<b>Консультация по стратегии участия</b>
• Ваша выгода: Бесплатно в течение 30 дней после завершения курса.
• Обсудите с нашим специалистом первые шаги, выбор тендеров и план действий для вашей компании.

<b>Как воспользоваться:</b>
Свяжитесь с нами по телефону +7 (4922) 223-222 или напишите на info@tritika.ru, указав в теме письма «Выпускник экспресс-курса, ФИО».

Ваш следующий шаг — ДЕЙСТВИЕ. Теперь у вас есть знания, поддержка и выгодные условия для первых побед. Мы будем рады помочь вам в этом!

Команда «Тритики» желает вам успешных тендеров и выгодных контрактов!"""
...
async def finish_test(message: Message, state: FSMContext):
    ...
    # Сохраняем прогресс после добавления результатов теста
    save_user_progress()
...
"""
    ...
    # ДОБАВЛЯЕМ ПОЗДРАВЛЕНИЕ С УСПЕШНЫМ ПРОХОЖДЕНИЕМ КУРСА И ТЕСТА
    if correct_answers >= 5:  # Если прошел тест успешно
        result_text += "\n\n🎉 <b>ПОЗДРАВЛЯЕМ С УСПЕШНЫМ ПРОХОЖДЕНИЕМ КУРСА!</b> 🎉"
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user_id not in user_progress:
        ...
        # Сохраняем прогресс нового пользователя
        save_user_progress()
...
🚀 <b>Курс включает:</b>
• 7 модулей с аудио-сопровождением
• Практические задания
• Финальный тест
• Чек-лист для работы
...
💰 <b>Стоимость:</b> 
   ...
   <b>*Акция до конца января 2026 года
...
async def handle_get_access(message: Message):
    ...
    # Проверяем, не имеет ли пользователь уже доступ
    ...
    # Формируем сообщение с информацией об оплате (упрощенное и читабельное)
    access_info = f"""
...
✅ <b>7 модулей с аудио-сопровождением:</b>
...
"""
    ...
    if os.path.exists(qr_code_path):
        try:
            ...
            caption += "✅ После оплаты отправьте чек в чат"
        ...
        # Если нет QR-кода, отправляем реквизиты
        await message.answer(
...
💰 <b>Тип доступа:</b> Полный курс (3 999 руб. по акции)
...
✅ <b>7 структурированных модулей:</b>
...
<b>💰 Оплата и доступ:</b>
• <b>🔓 Получить доступ</b> - оплата курса (3 999 руб. по акции до конца января 2026)
...
async def handle_mark_all_modules(message: Message):
    ...
    # Сохраняем прогресс после отметки всех модулей
    save_user_progress()
...
"""
    ...
    # ДОБАВЛЯЕМ ПОЗДРАВЛЕНИЕ, ЕСЛИ ТЕСТ ПРОЙДЕН УСПЕШНО
    if last_test['correct_answers'] >= 5:  # Если прошел тест успешно
        ...
        if audio_sent:
            if user_id in user_progress:
                if current_module + 1 not in user_progress[user_id].get('audio_listened', []):
                    ...
                    # Сохраняем прогресс после отметки прослушанного аудио
                    save_user_progress()
...
async def handle_complete_lesson(message: Message, state: FSMContext):
    ...
    if current_module is not None:
        ...
        if module_num not in user_progress[user_id]['completed_modules']:
            user_progress[user_id]['completed_modules'].append(module_num)
            # Сохраняем прогресс после отметки модуля
            save_user_progress()
...
• user_progress.json: {'✅ Существует' if os.path.exists(USER_PROGRESS_FILE) else '❌ Не существует'}
"""
...
<b>💰 Цена курса:</b>
• Обычная: 7 500 руб.
• Акционная (до 01.2026): 3 999 руб.
• После акции: 4 999 руб.
...
async def handle_other_messages(message: Message, state: FSMContext):
    ...
    if message.content_type == ContentType.TEXT:
        if access_control.is_paid_user(user_id):
            ...
            # ИЗМЕНЕНО: ПЕРЕНЕСЛИ ИНФОРМАЦИЮ ОБ АКЦИИ СЮДА
            await message.answer(
...
3. Оплатите курс
4. Отправьте чек об оплате в этот чат
...
async def run_bot_with_retries():
    ...
    while not shutdown_flag and restart_count < max_restarts:
        try:
            ...
            logger.info(f"✅ Прогресс пользователей загружен: {len(user_progress)} пользователей")
...
if __name__ == "__main__":
    try:
        ...
        print(f"📚 Прогресс пользователей загружен: {len(user_progress)}")
        ...
        print("=" * 60)
        print("🔐 ВАЖНЫЕ ОСОБЕННОСТИ СИСТЕМЫ:")
        ...
        print("• Информация об акции перенесена в сообщение о недоступном функционале")
        print("• Добавлено поздравление после успешного прохождения теста")
        print("• Добавлен сайт Тритика в контакты")
        print("• Добавлена инструкция как нажать кнопку '🔓 Получить доступ'")
        print("• Прогресс пользователей сохраняется в файл user_progress.json")
        print("• Добавлен новый модуль 5: Банковские гарантии в тендерах")
        print("• Старые модули 5 и 6 стали модулями 6 и 7")
        print("• После прохождения курса добавляется информация о подарках для выпускников")
        print("=" * 60)
